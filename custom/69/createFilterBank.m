...
function [filterBank] = createFilterBank()
    ...
    %Average
    for hsize=3:4
        filterBank = [filterBank, getAverageFilter(hsize)];
    end
    %sobel
    h =  fspecial('sobel');
    filterBank = [filterBank,h];
    %Laplacian
    for s=0:0.2:1
        filterBank = [filterBank, getLaplacianFilter(s)];
    end
end
...
function h = getAverageFilter(hsize)
    h = fspecial('average',hsize);
end

function h = getLaplacianFilter(s)
    h = fspecial('laplacian',s);
end
